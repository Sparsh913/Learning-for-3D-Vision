...
def get_pixels_from_image(image_size, camera):
    ...
    # pass
    # x, y = 2 * (x / (W - 1)) - 1, 2 * (y / (H - 1)) - 1
    x = torch.arange(-1, 1, 2 / W)
    y = torch.arange(-1, 1, 2 / H)
    x = x.to(device=camera.device)
    y = y.to(device=camera.device)
    ...
    return -xy_grid.to(device=camera.device)
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    # pass
    xy_grid_sub = xy_grid[torch.randperm(xy_grid.shape[0])].reshape(-1, 2)

    # Return
    return xy_grid_sub[:n_pixels]
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    ndc_points = torch.cat([ndc_points, torch.ones_like(ndc_points[..., -1:])], dim=-1) # [N, 3]
    ndc_points = ndc_points.to(device=camera.device)
    # print("ndc_points", ndc_points.shape)
