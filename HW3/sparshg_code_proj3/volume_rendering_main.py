...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            plt.imsave(f'images/xy_grid_{cam_idx}.png', img)
        del xy_grid
        torch.cuda.empty_cache()
        ...
        if cam_idx == 0 and file_prefix == '':
            # pass
            img_rays = vis_rays(ray_bundle, image_size)
            plt.imsave(f'images/rays_{cam_idx}.png', img_rays)
        
        # TODO (Q1.4): Implement point sampling along rays in sampler.py
        # pass
        
        model.sampler.forward(ray_bundle)
        # print("I have exited sampler.py")
        ...
        if cam_idx == 0 and file_prefix == '':
            # pass
            print("I am about to enter render_points")
            render_points(f'images/sample_points_{cam_idx}.png', ray_bundle.sample_points.view(-1,3).unsqueeze(0), image_size)

        # TODO (Q1.5): Implement rendering in renderer.py
        ray_bundle = model.sampler(ray_bundle)
        # print("checking ray bundle sample lengths", ray_bundle.sample_lengths)
...
def render(
    ...
    imageio.mimsave('images/part_1.gif', [np.uint8(im * 255) for im in all_images], fps = 10, loop=5000)
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            loss = None
            # MSE between predicted colors and ground truth colors
    ...
    imageio.mimsave('images/part_2.gif', [np.uint8(im * 255) for im in all_images], fps = 10, loop=5000)
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            out = model(ray_bundle)
            print("Model forward run output received")
        ...
        if (
            ...
            with torch.no_grad():
                ...
                imageio.mimsave('images/part_8.2.gif', [np.uint8(im * 255) for im in test_images], fps = 10, loop=5000)
