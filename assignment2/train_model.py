...
def train_model(args):
    ...
    wandb.init(
    project="Learning for 3D Vision HW2", 
    entity="sparshg",
    config = {
            ...
            try:
                for tag, value in model.named_parameters():
                    if value.grad is not None:
                        # value.grad should be float
                        value.grad = value.grad.float()
                        writer.add_histogram(tag + '/grad', value.grad.cpu().numpy(), step)
            except:
                print("Ignoring error in logging gradients")
                pass
