...
class SingleViewto3D(nn.Module):
    def __init__(self, args):
        ...
        if args.type == "vox":
            ...
            self.layer6 = nn.Sequential(
                nn.Conv3d(8, 1, kernel_size=1, bias=False),
                nn.Sigmoid()
            )
            ...
            '''
            Model parametric function that takes in 2 inputs:
            1. image embedding b x 512
            2. a 2d point with x, y ranging from 0 to 1
            '''
            
            # sample random x, y from 0 to 1
            # x, y = torch.rand(2)
            # Convert x, y to an embedding of size 512 using a linear layer
            self.point2emb = nn.Sequential(
                nn.Linear(2, 512)
            )
            
            # Let's define a simple MLP that takes in the concatenated embeddings and outputs a 3D point
            self.layer1 = nn.Sequential(
                    nn.Linear(512, 512)
                )
            self.act1 = nn.LeakyReLU()
    ...
    def forward(self, images, args):
        ...
        if args.type == "vox":
            ...
            voxels_pred = voxels_pred.view(-1, 256, 2, 2, 2) # b x 256 x 2 x 2 x 2
            ...
            # sample random x, y from 0 to 1 in the shape b x 2the image embedding and the point embedding
            # self.concat = nn.Sequential(
            sample_pts = torch.rand(B, 2).to(self.device) # shape b x 2
            # Convert x, y to an embedding of size 512 using a linear layer
            point_emb = self.point2emb(sample_pts) # shape b x 512
            # Add the image embedding and the point embedding to get a shape of b x 512
            point_emb = point_emb + encoded_feat
            # pass the concatenated embeddings through the MLP
            pointclouds_pred = self.layer1(point_emb)
            residue = pointclouds_pred
            pointclouds_pred = self.act1(pointclouds_pred)
            pointclouds_pred = self.layer2(pointclouds_pred)
            pointclouds_pred += residue
            pointclouds_pred = self.act2(pointclouds_pred)
            pointclouds_pred = self.layer3(pointclouds_pred)
            pointclouds_pred = pointclouds_pred.view(-1, self.n_point, 3)
