...
class SDS:
    ...
    def sds_loss(
        ...
        with torch.no_grad():
            ...
            noise_t = self.unet(noisy_latents, t, text_embeddings)
            if text_embeddings_uncond is not None and guidance_scale != 1:
                ### YOUR CODE HERE ###
                # pass
                # incorporate unconditional text embedding
                noise_t.sample += guidance_scale*(noise_t.sample - self.unet(noisy_latents, t, text_embeddings_uncond,).sample)
                
        # Compute SDS loss
        w = 1 - self.alphas[t]
        # target = latents+w * (noise_t - latents)
        # target_x = self.vae.decode(target.type(self.precision_t)).sample
        grad = w * (noise_t.sample - noise) * grad_scale
        ...
        if image_instead:
            with torch.no_grad():
                target = (latents - grad)
                latents_t = 1 / self.vae.config.scaling_factor * (target)
                target = self.vae.decode(latents_t.type(self.precision_t))

            latents = 1 / self.vae.config.scaling_factor * (latents)
            image = self.vae.decode(latents.type(self.precision_t)).sample
            loss = F.mse_loss(image, target.sample)
        else:
            with torch.no_grad():
                target = (latents - grad)
