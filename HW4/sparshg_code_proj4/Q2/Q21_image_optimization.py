...
def optimize_an_image(
    ...
    for i in tqdm(range(total_iter)):
        ...
        if args.sds_guidance:
            loss = SDS.sds_loss(sds, latents, embeddings['default'], embeddings['uncond'], guidance_scale=100, grad_scale=1, image_instead=True)
        else:
            loss = SDS.sds_loss(sds, latents, embeddings['default'], guidance_scale=0, grad_scale=1, image_instead=True)
