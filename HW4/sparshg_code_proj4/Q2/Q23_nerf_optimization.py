    ...
    for epoch in range(max_epoch):
        ...
        for data in train_loader:
            ...
            if not args.view_dep_text:
                text_cond = embeddings["default"] # shape
            else:
                ### YOUR CODE HERE ###
                # pass
                # 3 views: front, back, side
                # view selection based on azimuth
                # weight based on azimuth
                # interpolate text_cond based on azimuth
                # considering -180 is on the left side and starts clockwise from there
                if azimuth > -180 and azimuth < -90:
                    text_cond = -(torch.cos(azimuth * np.pi/180) * embeddings["side"] + torch.sin(azimuth * np.pi/180) * embeddings["front"])
                elif azimuth >= -90 and azimuth < 0:
                    text_cond = (torch.cos(azimuth * np.pi/180) * embeddings["side"] - torch.sin(azimuth * np.pi/180) * embeddings["front"])
                elif azimuth >= 0 and azimuth < 90:
                    text_cond = (torch.cos(azimuth * np.pi/180) * embeddings["side"] + torch.sin(azimuth * np.pi/180) * embeddings["back"])
                else:
                    text_cond = -(torch.cos(azimuth * np.pi/180) * embeddings["side"] - torch.sin(azimuth * np.pi/180) * embeddings["back"])
