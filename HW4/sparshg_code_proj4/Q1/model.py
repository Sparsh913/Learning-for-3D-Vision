...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        # convert quats to rotation matrices using pytorch3d functions
        rot = pytorch3d.transforms.quaternion_to_matrix(quats) # (N, 3, 3)
        
        if self.is_isotropic:

            ### YOUR CODE HERE ###
            cov_3D = None  # (N, 3, 3)
            # cov_3D = rot @ scales @ scales.T @ rot.T
            # cov_3D = torch.einsum('bij,bj,bj->bij', rot, scales, scales)  # (N, 3, 3)
            # scales is (N, 1). For each scale, we need to convert to 3,3 diagonal matrix
            
            # cov_3D = torch.zeros(scales.shape[0], 3, 3).to(scales.device) 
            # batch_size = scales.shape[0]
            # for i in range(batch_size):
            #     s = torch.eye(3).to(scales.device) * scales[i]
            #     cov_3D[i] = rot[i] @ s @ s.T @ rot[i].T
                
            # Vectorize the above code to make it more efficient
            # cov_3D = rot @ torch.diag_embed(scales) @ rot.T
            # shape of scales[:, None, None] is (N, 1, 1, 1)
            s = torch.eye(3).to(scales.device) * scales[:, None, None] # (N, 1, 3, 3)
            rot_exp = rot.unsqueeze(1) # (N, 1, 3, 3)
            cov_3D = rot_exp @ s**2 @ rot_exp.permute(0, 1, 3, 2) # (N, 1, 3, 3) @ (N, 1, 3, 3) @ (N, 1, 3, 3)
            # shape of cov_3D is (N, 1, 3, 3)
            cov_3D = cov_3D.squeeze(1) # (N, 3, 3)
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            ### YOUR CODE HERE ###
            cov_3D = None  # (N, 3, 3)
            # now gaussians are anisotropic, scaling matrix is of the form diag(scales)
            s = torch.diag_embed(scales) # (N, 3, 3)
            s = s.unsqueeze(1) # (N, 1, 3, 3)
    ...
    def compute_means_2D(means_3D: torch.Tensor, camera: PerspectiveCameras):
        ...
        means_2D = None  # (N, 2)
        # Gaussians are in world space, we need to convert them to camera space
        # print("means3D", means_3D.shape)
        # means_3D = camera.get_world_to_view_transform().transform_points(means_3D)
        # # Find camera projection matrix
        # projection_matrix = camera.get_projection_transform() # shape (N, 4, 4)
        # # print("projection_matrix", projection_matrix)
        # # extract matrix from the projection matrix object
        # projection_matrix = projection_matrix.get_matrix()[:, :3, :4] # shape (1, 3, 4)
        # #
        # projection_matrix = camera.compute_projection_matrix() # shape (N, 3, 4)
        # # Add batch to projection matrix
        # projection_matrix = projection_matrix.repeat(means_3D.shape[0], 1, 1) # shape (N, 3, 4)
        # print("projection_matrix", projection_matrix.shape)
        # # Convert 3D gaussians to camera space
        # # create a homogeneous coordinate by adding 1 to the last dimension in means_3D
        # means_3D_homogeneous = torch.cat((means_3D, torch.ones(means_3D.shape[0], 1).to(means_3D.device)), dim=1)
        # print("means_3D_homogeneous", means_3D_homogeneous.shape) # shape (N, 4)
        
        # # Convert 3D gaussians to camera space
        # screen_space = torch.matmul(projection_matrix, means_3D_homogeneous.unsqueeze(-1)) # shape (N, 3, 1)
        # print("screen_space", screen_space.shape)
        
        # screen_space = screen_space.squeeze(-1) # shape (N, 3)
        # # Normalize the screen space coordinates
        # screen_space = screen_space[:, :2] / screen_space[:, 2].unsqueeze(-1) # shape (N, 2)
        # means_2D = screen_space
        # print("means_2D", means_2D.shape)
        
        # Convert 3D gaussians from world space to camera space
        means_3D = camera.get_world_to_view_transform().transform_points(means_3D)
        # Normalize means_3D
        means_3D = -means_3D / means_3D[:, 2].unsqueeze(-1) # shape (N, 3)
        means_3D[:, 2] = 1.0
        # Find the camera intrinsic matrix
        f_x = camera.focal_length[0, 0]
        f_y = camera.focal_length[0, 1]
        c_x = camera.principal_point[0, 0]
        c_y = camera.principal_point[0, 1]
        K = torch.tensor([[f_x, 0, c_x], [0, f_y, c_y], [0, 0, 1]]).to(means_3D.device)

        means_2D = torch.matmul(K, means_3D.T).T # shape (N, 3) @ (3, 3) = (N, 3)
        means_2D = means_2D[:, :2] / means_2D[:, 2].unsqueeze(-1)
        return means_2D
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        cov_2D_inverse = cov_2D_inverse.unsqueeze(1)  # Shape: (N, 1, 2, 2)
        # power = -0.5 * (points_2D - means_2D) @ cov_2D_inverse @ (points_2D - means_2D).permute(0, 2, 1)
        power = -0.5 * torch.matmul(torch.matmul(diff.unsqueeze(-2), cov_2D_inverse), diff.unsqueeze(-1)).squeeze(-1).squeeze(-1)
        # print("power before squeeze", power.shape)
        # Power should be (N, H*W)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        means_3D = means_3D.to(self.device)
        # use the camera to calculate the depth of each gaussian
        camera = camera.to(self.device)
        means_view_space = camera.transform_points(means_3D) # shape (N, 3)
        z_vals = 1./means_view_space[:, 2]
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        # print("cov_2D_inverse", cov_2D_inverse.shape)
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        alphas = None  # (N, H*W) alpha value represents the transparency of the gaussian
    ...
    def compute_transmittance(
        ...
        # transmittance = cumprod(one_minus_alphas_j) where j < i
        transmittance = torch.cumprod(one_minus_alphas, dim=0)  # (N+1, H, W)
    ...
    def splat(
        ...
        # HINT: Refer to README for a relevant equation
        image = None  # (H, W, 3)
        # image = summation(colours * alphas * transmittance) # colors is (N, 1, 1, 3), alphas is (N, H, W, 1), transmittance is (N, H, W, 1)
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        camera_origin_in_world_frame = camera.get_world_to_view_transform().get_matrix()[:, :3, 3]
        gaussian_dirs = means_3D - camera_origin_in_world_frame
